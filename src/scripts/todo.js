...
let data = JSON.parse(localStorage.getItem("todo-multilist")) || {
  "My List": [],
};
...
function renderLists() {
  ...
  Object.keys(data).forEach((list) => {
    ...
    div.className =
      (list === currentList ? " bg-background-3" : "") +
      " rounded p-2 flex justify-between hover:brightness-80";
...
function renderTasks() {
  ...
  (data[currentList] || []).forEach((task, i) => {
    ...
    li.className +=
      " bg-background-1 rounded p-2 flex justify-between hover:brightness-80 group";
    ...
    editBtn.innerHTML = "✏️";
    editBtn.className =
      "edit-task opacity-0 group-hover:opacity-100 transition ml-2";
    ...
    editBtn.onclick = function (e) {
      ...
      input.className =
        "bg-transparent border-b border-zinc-400 outline-none flex-1";
